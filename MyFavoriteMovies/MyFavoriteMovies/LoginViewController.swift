...
class LoginViewController: UIViewController {
    ...
    private func getRequestToken() {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            guard error == nil else{
                displayError(error: "\(String(describing: error))")
                return
            }
            ...
            do {
                ...
                displayError(error: "json parse fail")
                return
            }
            
            /* GUARD: Did TheMovieDB return an error? */
            if let _ = parsedResult[Constants.TMDBResponseKeys.StatusCode] as? Int {
                displayError(error: "TheMovieDB returned an error. See the '\(Constants.TMDBResponseKeys.StatusCode)' and '\(Constants.TMDBResponseKeys.StatusMessage)' in \(String(describing: parsedResult))")
                return
            }
            
            
            guard let requestToken = parsedResult[Constants.TMDBParameterKeys.RequestToken] as? String else{
                displayError(error: "no key in json result")
                return
            }
    ...
    private func loginWithToken(_ requestToken: String) {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            guard (error == nil ) else{
                displayError("there was an error : \(String(describing: error))")
                return
            }
    ...
    private func getSessionID(_ requestToken: String) {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            /* GUARD: Did TheMovieDB return an error? */
            if let _ = parsedResult[Constants.TMDBResponseKeys.StatusCode] as? Int {
                displayError("TheMovieDB returned an error. See the '\(Constants.TMDBResponseKeys.StatusCode)' and '\(Constants.TMDBResponseKeys.StatusMessage)' in \(String(describing: parsedResult))")
                return
            }
            
            guard let sessionID = parsedResult[Constants.TMDBResponseKeys.SessionID] as? String else{
                displayError("no session id key in json response")
                return
            }
    ...
    private func getUserID(_ sessionID: String) {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            guard let userID = parsedResult![Constants.TMDBResponseKeys.UserID] as? Int else {
                displayError("Cannot find key '\(Constants.TMDBResponseKeys.UserID)' in \(String(describing: parsedResult))")
                return
            }
