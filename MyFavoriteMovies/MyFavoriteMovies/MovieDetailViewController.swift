...
class MovieDetailViewController: UIViewController {
    ...
    @IBAction func toggleFavorite(_ sender: AnyObject) {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request as URLRequest){ (data, response, error) in
            ...
            guard (error == nil) else{
                print("there was an error with you request:\(String(describing: error))")
                return
            }
            /* GUARD: Did we get a successful 2XX response? */
            guard let statusCode = (response as? HTTPURLResponse)?.statusCode, statusCode >= 200 && statusCode <= 299 else {
            ...
            guard (parsedResult[Constants.TMDBResponseKeys.StatusCode] as? Int) != nil else {
                print("Could not find key '\(Constants.TMDBResponseKeys.StatusCode)' in  \(String(describing: parsedResult))")
                return
            }
            ...
            if shouldFavorite && !(tmdbStatusCode == 12 || tmdbStatusCode == 1){
                print("Unrecognized '\(Constants.TMDBResponseKeys.StatusCode)' in  \(String(describing: parsedResult))")
                return
